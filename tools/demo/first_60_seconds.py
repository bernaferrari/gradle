    ...
    try:
        ...
        def parse_run(output: str) -> dict[str, object]:
            ...
            return {
                ...
                "tasks_skipped": int(executed_match.group(3)) if executed_match and executed_match.lastindex and executed_match.lastindex >= 3 and executed_match.group(3) else 0,
                "tasks_from_cache": int(executed_match.group(4)) if executed_match and executed_match.lastindex and executed_match.lastindex >= 4 else 0,
        ...
        return {
            ...
            "first_tasks_skipped": first_parsed["tasks_skipped"],
            "warm_tasks_skipped": warm_parsed["tasks_skipped"],
...
def write_markdown_report(path: Path, results: list[dict[str, object]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# Rust Substrate First-60s Metrics",
        "",
        "This report is generated by `tools/demo/first_60_seconds.py`. It is a preview evidence artifact, not a universal Gradle performance claim.",
        "",
        "| Metric | Status | Elapsed | Threshold | Key evidence |",
        "| --- | --- | ---: | ---: | --- |",
    ]
    for result in results:
        status = "SKIP" if result.get("skipped") else ("PASS" if result["ok"] else "FAIL")
        threshold = result.get("threshold_ms")
        threshold_text = f"{threshold} ms" if threshold is not None else ""
        evidence = ""
        if result["name"] == "daemon_socket_ready":
            evidence = f"daemon self-reported {result.get('daemon_reported_ready_ms')} ms"
            ...
            evidence = (
                f"remote requests avoided {result.get('remote_requests_avoided')}, "
                f"dynamic output `{result.get('output_sha256')}`, "
                f"static output `{result.get('static_prefetch_output_sha256')}`"
            )
        elif result["name"] == "installed_authoritative_file_watch" and not result.get("skipped"):
            evidence = (
                f"Rust daemon observed {result.get('rust_daemon_observed')}, "
                f"Gradle watching active {result.get('watching_active')}"
            )
        elif result.get("runtime_metric_ms") is not None:
            evidence = f"runtime metric {result.get('runtime_metric_ms')} ms"
        elif result.get("test_filter"):
            evidence = f"test `{result.get('test_filter')}`"
        elif result.get("test_filters"):
            evidence = f"{len(result.get('test_filters', []))} Gradle seam tests"
        elif result.get("tail"):
            evidence = str(result["tail"]).splitlines()[-1] if str(result["tail"]).splitlines() else ""
        lines.append(
            f"| `{result['name']}` | {status} | {result.get('elapsed_ms', '')} ms | {threshold_text} | {evidence} |"
        )

    failed = [str(result["name"]) for result in results if not result["ok"]]
    lines.extend(["", "## Outcome", ""])
    if failed:
        lines.append("The run is not a passing preview artifact.")
        lines.append("")
        lines.append("Failing metrics: " + ", ".join(f"`{name}`" for name in failed))
    else:
        lines.append("All reported metrics passed their configured preview thresholds.")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    parser.add_argument("--markdown-output", type=Path, help="write Markdown metrics summary to this path")
    ...
    if args.markdown_output:
        write_markdown_report(args.markdown_output, results)
        print(f"Metrics Markdown: {args.markdown_output}")
