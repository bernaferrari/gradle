...
pub fn apply_vfs_delta_to_write_file(
    ...
    for (changed_path, _hash) in delta_child_summaries.iter() {
        ...
        {
            tracing::info!(target: "writefile-lowering", vfs_taskexec_cross = true, target = %target_path.display(), changed = %changed_path, "VFS delta intersects write-file output — re-execution likely required");
            return true;
        }
    }
    false
