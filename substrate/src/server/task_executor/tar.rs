...
pub fn apply_vfs_delta_to_tar_archive(
    ...
    for (changed_path, _hash) in delta_child_summaries.iter() {
        ...
        {
            tracing::info!(target: "tar-lowering", vfs_taskexec_cross = true, archive = %archive_path.display(), changed = %changed_path, "VFS delta intersects tar inputs — re-execution likely required");
            return true;
        }
    }
    false
