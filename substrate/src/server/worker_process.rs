...
impl WorkerProcessServiceImpl {
    ...
    pub fn apply_vfs_delta_to_workers(&self, _delta: &std::collections::HashMap<String, Vec<u8>>) {
        let paths: BTreeSet<String> = std::collections::BTreeSet::new();
        if paths.is_empty() {
            return;
        }
        tracing::info!(
            target: "workers",
            paths = ?paths.len(),
            "VFS delta cross applied to worker pool/lease/heartbeat/healthy (DirectorySnapshot child_summaries BTree)"
        );
    }

    #[allow(dead_code)]
    fn result_channel_v1_for_workers(&self) {
        tracing::debug!(target: "workers", "result channel v1 workers (BTree child_summaries; det lifecycle/lease/healthy/pool)");
    }
