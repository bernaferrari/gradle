...
async fn test_hash_determinism_across_repeated_calls() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let file_path = make_test_file(dir.path(), "deterministic.txt", b"deterministic content for hash verification");
    ...
    for i in 1..hashes.len() {
        assert_eq!(hashes[0], hashes[i], "Hash at iteration {} differs from first", i);
    }
}
...
async fn test_hash_different_files_produce_different_hashes() {
    ...
    let response = client
        .hash_batch(Request::new(HashBatchRequest {
            files: vec![
                FileToHash { absolute_path: file_a, length: 0, last_modified: 0 },
                FileToHash { absolute_path: file_b, length: 0, last_modified: 0 },
                FileToHash { absolute_path: file_c, length: 0, last_modified: 0 },
            ],
            algorithm: String::new(),
        }))
        .await
    ...
    assert_eq!(response.results[0].hash_bytes, response.results[2].hash_bytes);
    // A and B must have different hashes
    assert_ne!(response.results[0].hash_bytes, response.results[1].hash_bytes);
}
...
async fn test_cache_key_determinism() {
    ...
    assert_eq!(key1.cache_key, key2.cache_key, "Identical inputs must produce identical cache keys");
    ...
    assert_ne!(key1.cache_key, key3.cache_key, "Different implementation hash must produce different cache key");
}
...
async fn test_value_snapshot_determinism() {
    ...
    let request = SnapshotValuesRequest {
        values: vec![
            PropertyValue {
                ...
                value: Some(property_value::Value::StringValue("src/main/java".to_string())),
    ...
    assert_eq!(resp1.composite_hash, resp2.composite_hash, "Composite hash must be deterministic");
    assert_eq!(resp1.results.len(), resp2.results.len());
    for (r1, r2) in resp1.results.iter().zip(resp2.results.iter()) {
        assert_eq!(r1.fingerprint, r2.fingerprint, "Fingerprint for {} must be deterministic", r1.name);
    }
}
...
async fn test_file_fingerprint_determinism() {
    ...
    assert_eq!(resp1.collection_hash, resp2.collection_hash, "Collection hash must be deterministic");
...
async fn test_execution_plan_consistency() {
    ...
    let channel = connect(&socket_path).await;
    let mut client =
        execution_plan_service_client::ExecutionPlanServiceClient::new(channel);
    ...
    assert_eq!(resp1.predicted_outcome, resp2.predicted_outcome,
...
async fn test_build_comparison_determinism() {
    ...
    let mut c_rec = build_comparison_service_client::BuildComparisonServiceClient::new(channel.clone());
    for build_id in ["baseline-a", "baseline-b"] {
        c_rec.record_build_data(Request::new(RecordBuildDataRequest {
    ...
    assert_eq!(result1.summary.unwrap().tasks_with_regression, 0,
        "Identical builds should have zero regressions");
    assert!(result1.task_comparisons.len() >= 2);

    for tc in &result1.task_comparisons {
        assert_eq!(tc.duration_ratio, 1.0,
            "Task {} should have ratio 1.0 for identical builds", tc.task_path);
    }
}
...
async fn test_plugin_apply_order_determinism() {
    ...
    let setup_and_get_plugins = |ch: Channel| async {
        ...
        let project = format!("det-plugin-{}", std::time::SystemTime::now()
...
async fn test_config_cache_roundtrip_determinism() {
    ...
    client.store_config_cache(Request::new(store_req.clone()))
        .await
        .unwrap();
    ...
    client.store_config_cache(Request::new(store_req))
        .await
        .unwrap();
...
async fn test_build_metrics_aggregation_determinism() {
    ...
    let record_and_get = |ch: Channel| async {
        ...
        let build_id = format!("det-metrics-{}", std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_nanos());
...
async fn test_full_build_lifecycle_determinism() {
    ...
    let run_lifecycle = |ch: Channel| async move {
        let build_id = format!("det-lifecycle-{}", std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_nanos());
