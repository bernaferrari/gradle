...
async fn test_linear_chain_topological_order() {
    ...
    assert!(errors.is_empty(), "Topological order errors:\n{}", errors.join("\n"));

    // Verify the strict order A, B, C, D
    let order: Vec<&str> = plan.execution_order.iter().map(|n| n.task_path.as_str()).collect();
    assert_eq!(order, vec![":A", ":B", ":C", ":D"], "Linear chain must execute in exact order");
}
...
async fn test_large_dag_topological_order() {
    ...
    let tasks = vec![
        ...
        (":compileTestJava", "JavaCompile", vec![":processResources".to_string()]),
        (":testClasses", "Lifecycle", vec![":compileTestJava".to_string()]),
        (":test", "Test", vec![":testClasses".to_string()]),
        (":check", "Lifecycle", vec![":test".to_string()]),
        (":build", "Lifecycle", vec![":jar".to_string(), ":check".to_string()]),
        (":clean", "Delete", vec![]),
        ...
        (":verify", "Lifecycle", vec![":check".to_string(), ":lint".to_string()]),
    ];
    ...
    assert_eq!(plan.execution_order.len(), 15, "All 15 tasks should be in execution order");
...
async fn test_cycle_detection() {
    ...
    assert!(ref_has_cycle, "Reference implementation should also detect the cycle");
}
...
async fn test_dag_executor_full_build_completion() {
    ...
    let channel = connect(&socket_path).await;
    let mut tg_client = task_graph_service_client::TaskGraphServiceClient::new(channel.clone());
    let mut dag_client = dag_executor_service_client::DagExecutorServiceClient::new(channel.clone());
    ...
    let errors = verify_topological_order(
        ...
        &executed_tasks
            ...
            .map(|(i, t)| ExecutionNode {
                task_path: t.clone(),
                dependencies: task_defs.iter().find(|(p, _)| p == t).map(|(_, d)| d.clone()).unwrap_or_default(),
                execution_order: (i + 1) as i64,
                estimated_duration_ms: 0,
            })
            .collect::<Vec<_>>(),
    );
    assert!(errors.is_empty(), "Execution order errors:\n{}", errors.join("\n"));
...
async fn test_build_isolation_separate_build_ids() {
    ...
    let paths1: Vec<&str> = plan1.execution_order.iter().map(|n| n.task_path.as_str()).collect();
    ...
    let paths2: Vec<&str> = plan2.execution_order.iter().map(|n| n.task_path.as_str()).collect();
...
async fn test_multiple_independent_roots() {
    ...
    assert_eq!(plan.ready_to_execute, 3, "Three independent roots should be ready");
...
async fn test_topological_sort_matches_reference_implementation() {
    ...
    let tasks = vec![
        ...
        (":mid2", "Mid", vec![":root1".to_string(), ":root2".to_string()]),
        (":mid3", "Mid", vec![":root2".to_string()]),
        (":leaf1", "Leaf", vec![":mid1".to_string(), ":mid2".to_string()]),
        (":leaf2", "Leaf", vec![":mid2".to_string(), ":mid3".to_string()]),
        (":leaf3", "Leaf", vec![":mid3".to_string()]),
        (":final1", "Final", vec![":leaf1".to_string(), ":leaf2".to_string()]),
        (":final2", "Final", vec![":leaf2".to_string(), ":leaf3".to_string()]),
    ];
