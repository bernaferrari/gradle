...
| Unsupported Maven local repositories | Rust dependency/repository transport does not yet model Gradle `mavenLocal()` semantics or the user-scoped `~/.m2` repository policy. The unsupported corpus now includes `maven-local-unsupported-kotlin-dsl`, a deterministic Java-library fixture with `repositories { mavenLocal() }` and no dependency on the user's local Maven cache. Contract scanning reports `maven-local`; JVM task contracts mark `maven-local:build-script` plus the captured `repository-url-scheme:MavenLocal:file`; and the authoritative single-entry manifest passed expected fail-closed checks with upstream successful, substrate exit 1 before dispatch, no no-op fallback, and `jvmForwarded=0`. This is intentionally fail-closed evidence only; native Maven local support is not claimed until Rust represents Gradle's local repository lookup and cache policy explicitly. | `python3 -m unittest tools.corpus_runner.test_run`; `python3 tools/corpus_runner/run.py --manifest testing/corpus/unsupported-manifest.json --contract-only --output-dir build/corpus-maven-local-contract`; `./gradlew :rust-bridge:compileJava :rust-bridge:compileTestJava -x :rust-bridge:extractIncludeTestProto -x :rust-bridge:generateTestProto --no-daemon --console=plain`; `./gradlew :distributions-full:install -Pgradle_installPath=$PWD/build/gradle-under-test -Dorg.gradle.unsafe.isolated-projects=false -Dorg.gradle.configuration-cache=false --no-daemon --console=plain`; `python3 tools/corpus_runner/run.py --manifest build/maven-local-single-manifest.json --gradle-command "$PWD/build/gradle-under-test/bin/gradle" --daemon-binary target/debug/gradle-substrate-daemon --runbuild-authoritative --tasks clean build --timeout 300 --output-dir build/corpus-maven-local-single-manifest --verbose` |
