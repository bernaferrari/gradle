...
| Kernel dependency admission tests | 7/7 focused kernel tests passed; full daemon library suite: 1642 passed, 0 failed, 3 ignored. | `cargo test -p gradle-substrate-daemon execution_kernel --lib`; `cargo test -p gradle-substrate-daemon --lib` |
| Kernel dependency graph bridge tests | 6/6 focused DAG conversion tests passed; DAG now delegates module selector notation parsing to the Rust dependency graph builder instead of owning a private parser. | `cargo test -p gradle-substrate-daemon kernel_dependency_graph --lib` |
...
| Unit tests | 1642 passed, 0 failed, 3 ignored | `cargo test -p gradle-substrate-daemon --lib` |
