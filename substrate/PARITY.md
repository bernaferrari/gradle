...
- Captured JVM-host build-plan shadows can drive native Rust `JavaCompile`
  execution with JVM fallback disabled when the task contract includes source,
  output, and toolchain inputs.
...
3. Add native-ready contracts for `ProcessResources`, `Classes`, `Jar`, `Copy`,
   and `Test` enough to complete a small Java library lifecycle.
