...
| External OSS dogfood strict gate | Added `testing/dogfood/oss-manifest.json` with immutable refs for Spring PetClinic, Mockito, and Okio, plus runner support for fetching git sources under `build/dogfood-oss/sources` and isolated upstream/substrate workdirs. Spring PetClinic now has a supported strict `clean compileJava` slice: concrete Rust RunBuild executed 2 tasks from `build-plan-cache` with 0 JVM forwards, task/output/hash/archive parity, upstream=6389ms and substrate=5528ms. The broader Spring `clean testClasses` slice is tracked as a separate fail-closed gate with a precise CycloneDX SBOM diagnostic because `:cyclonedxBom` and `:cyclonedxDirectBom` declare SBOM outputs and cannot be treated as lifecycle/no-op tasks. Mockito and Okio remain strict fail-closed with the precise JVM-owned `settings/includeBuild/buildSrc composite setup` diagnostic. Aggregate OSS dogfood evidence: 4/4 projects matched, 1/1 supported matched, 3/3 fail-closed matched, 1/1 supported zero JVM forwards, 1/4 Rust RunBuild executions, 4/4 task-graph captures, upstream task total 33, substrate task total 11, observed wall time upstream=35526ms and substrate=108959ms. | `cargo test -p gradle-substrate-daemon rejects_cyclonedx_sbom_tasks_with_precise_diagnostic --lib`; `cargo test -p gradle-substrate-daemon execution_kernel --lib`; `cargo test -p gradle-substrate-daemon test_project_dir_from_context_path_uses_nearest_settings_directory --lib`; `./gradlew :rust-bridge:compileJava :rust-bridge:compileTestJava -x :rust-bridge:extractIncludeTestProto -x :rust-bridge:generateTestProto --no-daemon --console=plain`; `./gradlew :distributions-full:install -Pgradle_installPath=$PWD/build/gradle-under-test -Dorg.gradle.unsafe.isolated-projects=false -Dorg.gradle.configuration-cache=false --no-daemon --console=plain`; `cargo build -p gradle-substrate-daemon`; `python3 -m unittest tools.dogfood_runner.test_run`; `python3 tools/dogfood_runner/run.py --manifest testing/dogfood/oss-manifest.json --execute --gradle-command "$PWD/build/gradle-under-test/bin/gradle" --daemon-binary target/debug/gradle-substrate-daemon --output-dir build/dogfood-oss --verbose`; artifacts: `build/dogfood-oss/dogfood-results.json`, `build/dogfood-oss/dogfood-summary.md`, `docs/rust-substrate-dogfood.md` |
