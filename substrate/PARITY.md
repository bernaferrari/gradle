...
- `testing/corpus/unsupported-manifest.json` tracks work that must remain
  ...
  covers custom JVM task actions, component metadata rules, and detached
  configurations, artifact views, artifact transforms, and composite
  substitution. Exact method-level Test includes, representable
  ...
  `enforcedPlatform("group:name:version")` use are now represented natively;
  wildcard or otherwise ambiguous method filters, general repository regex
  filters, non-module dependency substitution, and richer enforced-platform
  forms still belong in unsupported gates.
...
| Unsupported component metadata rules | Unsupported corpus now includes `component-metadata-rule-unsupported-kotlin-dsl`, a checked-in local Maven fixture using `dependencies { components { all { ... } } }`. Contract scanning reports `component-metadata-rule`; bridge task contracts mark component metadata rules as unsupported dependency semantics; and kernel rejection diagnostics now include captured unsupported feature names such as `component-metadata-rule:build-script` rather than only the generic marker. The authoritative expected-fail-closed run passed its checks with upstream successful, substrate exit 1 before dispatch, `fail_closed_message=true`, `component-metadata-rule` in the diagnostic, and `jvmForwarded=0`. This is intentionally fail-closed evidence only; component metadata rules are not claimed as native solver support. | `python3 tools/corpus_runner/run.py --manifest testing/corpus/unsupported-manifest.json --contract-only --output-dir build/corpus-component-metadata-unsupported-contract-final`; `python3 -m unittest tools.corpus_runner.test_run`; `./gradlew :rust-bridge:compileJava --no-daemon --console=plain`; `./gradlew :distributions-full:install -Pgradle_installPath=$PWD/build/gradle-under-test -Dorg.gradle.unsafe.isolated-projects=false -Dorg.gradle.configuration-cache=false --no-daemon --console=plain`; `cargo test -p gradle-substrate-daemon execution_kernel --lib`; `cargo build -p gradle-substrate-daemon --bin gradle-substrate-daemon`; `JAVA_TOOL_OPTIONS="-Dorg.gradle.rust.substrate.state.dir=$TMPDIR" python3 tools/corpus_runner/run.py --manifest <single-component-metadata-unsupported-manifest> --gradle-command "$PWD/build/gradle-under-test/bin/gradle" --daemon-binary target/debug/gradle-substrate-daemon --runbuild-authoritative --tasks clean build --timeout 300 --output-dir build/corpus-component-metadata-unsupported-only-final2 --verbose` |
...
| Corpus runner tests | 35 passed | `python3 -m unittest tools.corpus_runner.test_run` |
