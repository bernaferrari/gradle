...
- `testing/corpus/unsupported-manifest.json` tracks work that must remain
  ...
  covers custom JVM task actions, dependency substitution, component metadata
  rules, and detached configurations, artifact views, artifact transforms,
  composite substitution, and enforced platforms. Exact method-level Test
  includes, representable file-backed Maven repositories, and literal
  repository group regex filters are now represented natively; wildcard or
  otherwise ambiguous method filters and general repository regex filters still
  belong in unsupported gates.
...
| Unsupported dependency substitution | Unsupported corpus now includes `dependency-substitution-unsupported-kotlin-dsl`, a checked-in local Maven fixture where Gradle substitutes `org.gradle.substrate:original` with `org.gradle.substrate:replacement:1.0`. Contract scanning reports `dependency-substitution` and unsupported-manifest contract-only validation passed 4/4. The authoritative expected-fail-closed run passed its checks with upstream successful, substrate exit 1 before dispatch, `fail_closed_message=true`, and `jvmForwarded=0`. This is intentionally fail-closed evidence only; dependency substitution is not claimed as native solver support. | `python3 tools/corpus_runner/run.py --manifest testing/corpus/unsupported-manifest.json --contract-only --output-dir build/corpus-dependency-substitution-unsupported-contract-final2`; `./gradlew :rust-bridge:compileJava --no-daemon --console=plain`; `./gradlew :distributions-full:install -Pgradle_installPath=$PWD/build/gradle-under-test -Dorg.gradle.unsafe.isolated-projects=false -Dorg.gradle.configuration-cache=false --no-daemon --console=plain`; `JAVA_TOOL_OPTIONS="-Dorg.gradle.rust.substrate.state.dir=$TMPDIR" python3 tools/corpus_runner/run.py --manifest <single-dependency-substitution-unsupported-manifest> --gradle-command "$PWD/build/gradle-under-test/bin/gradle" --daemon-binary target/debug/gradle-substrate-daemon --runbuild-authoritative --tasks clean build --timeout 300 --output-dir build/corpus-dependency-substitution-unsupported-only-final2 --verbose`; `python3 -m unittest tools.corpus_runner.test_run` |
