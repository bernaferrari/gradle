...
`includeModule`, and `excludeModule` content filters are captured as native
repository filters and Rust skips repositories that do not admit a dependency
group/module pair. Other content filters are still marked unsupported, including
version, regex, configuration, and required-attribute gates. Custom repository
...
| Static repository module content filters | Proto/bridge regressions prove exact Gradle Maven `includeModule`/`excludeModule` content specs are captured as `group:module` repository filters, while versioned module specs, regex, configuration, and attribute filters remain fail-closed. Rust resolver regressions prove non-admitting exact-module repositories are skipped before POM fetch and artifact URL selection uses the admitted repository. Dependency resolution suite: 139 passed, 0 failed. Full daemon library suite: 1678 passed, 0 failed, 3 ignored. Java bridge dependency adapter suite passed. | `./gradlew :rust-bridge:test --tests org.gradle.internal.rustbridge.dependency.DependencyResolutionModelAdapterTest --no-daemon --console=plain`; `cargo test -p gradle-substrate-daemon test_repository_module_content_filter_skips_non_matching_repo --lib`; `cargo test -p gradle-substrate-daemon test_repository_exclude_module_content_filter_skips_repo --lib`; `cargo test -p gradle-substrate-daemon dependency_resolution::tests --lib`; `cargo test -p gradle-substrate-daemon --lib` |
