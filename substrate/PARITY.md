...
metadata-miss policy remains explicitly tracked because Gradle has artifact-only
and lenient-resolution edge cases that need separate parity work.
...
| External corpus after metadata gates | 7/7 matched, 88/88 task parity, 7/7 no-fallback, declared and resolved graph parity enabled, output/hash/archive parity passed after selected `.module` artifact files, resolved-repository artifact URLs, and direct missing-metadata fail-closed gates, observed wall time upstream=23657ms and substrate=24858ms | `python3 tools/corpus_runner/run.py --manifest testing/corpus/external-manifest.json --gradle-command "$PWD/build/gradle-under-test/bin/gradle" --daemon-binary target/debug/gradle-substrate-daemon --runbuild-authoritative --dependency-graph-parity --resolved-dependency-graph-parity --tasks clean build --timeout 300 --output-dir build/corpus-external-post-metadata-gates --verbose` |
