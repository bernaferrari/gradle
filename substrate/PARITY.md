...
- `testing/corpus/unsupported-manifest.json` tracks work that must remain
  ...
  configurations, artifact views, artifact transforms, composite substitution,
  and enforced platforms. Exact method-level Test includes, representable
  file-backed Maven repositories, literal repository group regex filters, and
  exact module-to-module dependency substitution are now represented natively;
  wildcard or otherwise ambiguous method filters, general repository regex
  filters, and non-module dependency substitution still belong in unsupported
  gates.
...
| BOM/platform/constraint current gate | Current bounded platform/constraint slice is verified without claiming full Gradle platform semantics. Ordinary Maven BOM via `platform(...)` passed 1/1 with 13/13 task parity, no fallback, declared and resolved graph parity, output/hash/archive parity, upstream=8041ms and substrate=4498ms. Static dependency constraints passed 1/1 with 13/13 task parity, no fallback, resolved graph parity, upstream=12059ms and substrate=4820ms. Unsupported `enforcedPlatform(...)` now has a checked-in local Maven fixture; upstream succeeds, Rust authoritative execution fails closed before dispatch with `enforced-platform:build-script`, `fail_closed_message=true`, and `jvmForwarded=0`. | `python3 tools/corpus_runner/run.py --project "$PWD/testing/corpus/dependency-platform-bom-kotlin-dsl" --gradle-command "$PWD/build/gradle-under-test/bin/gradle" --daemon-binary target/debug/gradle-substrate-daemon --runbuild-authoritative --dependency-graph-parity --resolved-dependency-graph-parity --tasks clean build --timeout 300 --output-dir build/corpus-bom-current --verbose`; `python3 tools/corpus_runner/run.py --project "$PWD/testing/corpus/dependency-constraints-kotlin-dsl" --gradle-command "$PWD/build/gradle-under-test/bin/gradle" --daemon-binary target/debug/gradle-substrate-daemon --runbuild-authoritative --dependency-graph-parity --resolved-dependency-graph-parity --tasks clean build --timeout 300 --output-dir build/corpus-constraints-current --verbose`; `./gradlew -p testing/corpus/enforced-platform-unsupported-kotlin-dsl clean build --no-daemon --console=plain`; `python3 tools/corpus_runner/run.py --manifest testing/corpus/unsupported-manifest.json --contract-only --output-dir build/corpus-enforced-platform-contract`; `python3 -m unittest tools.corpus_runner.test_run`; `./gradlew :rust-bridge:compileJava :rust-bridge:compileTestJava -x :rust-bridge:extractIncludeTestProto -x :rust-bridge:generateTestProto --no-daemon --console=plain`; `cargo test -p gradle-substrate-daemon execution_kernel --lib`; `./gradlew :distributions-full:install -Pgradle_installPath=$PWD/build/gradle-under-test -Dorg.gradle.unsafe.isolated-projects=false -Dorg.gradle.configuration-cache=false --no-daemon --console=plain`; `JAVA_TOOL_OPTIONS="-Dorg.gradle.rust.substrate.state.dir=$PWD/build/enforced-platform-fail-closed-state" python3 tools/corpus_runner/run.py --manifest <single-enforced-platform-manifest> --gradle-command "$PWD/build/gradle-under-test/bin/gradle" --daemon-binary target/debug/gradle-substrate-daemon --runbuild-authoritative --tasks clean build --timeout 300 --output-dir build/corpus-enforced-platform-unsupported-only --verbose` |
