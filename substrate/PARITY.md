...
- `testing/corpus/unsupported-manifest.json` tracks work that must remain
  ...
  covers custom JVM task actions, detached configurations, artifact views,
  artifact transforms, and composite substitution. Exact method-level Test
  ...
  component metadata normalization are now represented natively;
  ...
  and arbitrary component metadata rules still belong in unsupported gates.
...
| Unsupported detached configurations | Unsupported corpus now includes `detached-configuration-unsupported-kotlin-dsl`, a checked-in local Maven fixture that resolves `configurations.detachedConfiguration(...)` from a custom task and writes a deterministic output. Contract scanning reports `detached-configuration`; bridge task contracts mark detached configuration usage as unsupported dependency semantics; and the authoritative expected-fail-closed run passed with upstream successful, substrate exit 1 before dispatch, `fail_closed_message=true`, `detached-configuration` in the diagnostic, and `jvmForwarded=0`. This is intentionally fail-closed evidence only; detached configurations are not claimed as native solver support until the Rust graph builder represents them explicitly. | `./gradlew -p testing/corpus/detached-configuration-unsupported-kotlin-dsl clean build --no-daemon --console=plain`; `python3 tools/corpus_runner/run.py --manifest testing/corpus/unsupported-manifest.json --contract-only --output-dir build/corpus-detached-configuration-unsupported-contract`; `python3 -m unittest tools.corpus_runner.test_run`; `./gradlew :rust-bridge:compileJava --no-daemon --console=plain`; `./gradlew :distributions-full:install -Pgradle_installPath=$PWD/build/gradle-under-test -Dorg.gradle.unsafe.isolated-projects=false -Dorg.gradle.configuration-cache=false --no-daemon --console=plain`; `cargo test -p gradle-substrate-daemon execution_kernel --lib`; `JAVA_TOOL_OPTIONS="-Dorg.gradle.rust.substrate.state.dir=$TMPDIR" python3 tools/corpus_runner/run.py --manifest <single-detached-configuration-unsupported-manifest> --gradle-command "$PWD/build/gradle-under-test/bin/gradle" --daemon-binary target/debug/gradle-substrate-daemon --runbuild-authoritative --tasks clean build --timeout 300 --output-dir build/corpus-detached-configuration-unsupported-only --verbose` |
...
| Corpus runner tests | 37 passed | `python3 -m unittest tools.corpus_runner.test_run` |
