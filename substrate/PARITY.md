...
| CycloneDX bridge missing-contract marker | The JVM bridge now identifies `CyclonedxDirectTask` and `CyclonedxAggregateTask` as SBOM tasks instead of anonymous JVM work. Until complete SBOM IR capture exists, their task contracts carry `cyclonedx_sbom_contract_status=missing` and `requires_jvm_task_execution=true`, and deliberately do not emit `sbom_contract_json_b64`. This makes the missing bridge data explicit and test-covered while preserving fail-closed behavior. | `./gradlew :rust-bridge:test --tests org.gradle.internal.rustbridge.jvmhost.ProjectModelProviderAdapterTest.marksCycloneDxTaskAsMissingSchemaBackedSbomContract --no-daemon --console=plain -x :rust-bridge:extractIncludeTestProto -x :rust-bridge:generateTestProto`; `cargo test -p gradle-substrate-daemon execution_kernel --lib`; `cargo test -p gradle-substrate-daemon cyclonedx --lib`. |
