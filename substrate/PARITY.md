...
- The checked-in offline corpus covers Java library, Java application,
  ...
  Javadoc task, an exact detached-configuration resolved-file report, and an
...
- `testing/corpus/unsupported-manifest.json` tracks work that must remain
  ...
  covers custom JVM task actions, artifact views, artifact transforms, and
  composite substitution. Exact method-level Test
  ...
  component metadata normalization, and the exact detached-configuration
  resolved-file report fixture are now represented natively;
  ...
  arbitrary component metadata rules, and arbitrary detached-configuration
  task actions still belong in unsupported gates.
...
| Corpus runner tests | 39 passed | `python3 -m unittest tools.corpus_runner.test_run` |
