...
| Gradle Module Metadata unsupported fields | Parser regressions prove Rust fails closed when a selected JVM variant uses `available-at` redirects or dependency-constraint exclusions, rather than silently discarding those graph-affecting fields during serde parsing. Dependency resolution suite: 130 passed, 0 failed. Full daemon library suite: 1665 passed, 0 failed, 3 ignored. | `cargo test -p gradle-substrate-daemon gradle_module_metadata --lib`; `cargo test -p gradle-substrate-daemon dependency_resolution::tests --lib`; `cargo test -p gradle-substrate-daemon --lib` |
