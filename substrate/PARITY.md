...
| Maven repository-chain artifact selection | Unit regression proves first-repository POM miss followed by second-repository POM hit reports the resolved artifact URL from the second repository. Dependency resolution suite: 123 passed, 0 failed. Full daemon library suite: 1624 passed, 0 failed, 3 ignored. | `cargo test -p gradle-substrate-daemon test_maven_artifact_url_uses_repository_that_supplied_pom --lib -- --nocapture`; `cargo test -p gradle-substrate-daemon dependency_resolution --lib`; `cargo test -p gradle-substrate-daemon --lib` |
| Gradle Module Metadata JVM variant slice | Parser/service tests passed; Rust selects the `runtimeElements` dependency list and selected variant artifact file for an explicitly enabled Gradle metadata repository, does not fetch the root POM, avoids repeated child `.module` misses, and fails closed on custom capabilities. Full daemon library suite: 1624 passed, 0 failed, 3 ignored. | `cargo test -p gradle-substrate-daemon gradle_module_metadata --lib -- --nocapture`; `cargo test -p gradle-substrate-daemon dependency_resolution --lib`; `cargo test -p gradle-substrate-daemon --lib` |
