...
- `testing/corpus/unsupported-manifest.json` tracks work that must remain
  ...
  views, artifact transforms, composite substitution, and enforced platforms.
  Exact method-level Test includes and representable file-backed Maven
  repositories are now represented natively; wildcard or otherwise ambiguous
...
| Unsupported repository regex content filters | Unsupported corpus now includes `repository-regex-filter-unsupported-kotlin-dsl`, a checked-in local Maven fixture using `includeGroupByRegex`. Contract scanning reports `repository-content-filter` and unsupported-manifest contract-only validation passed 3/3. The authoritative expected-fail-closed run passed its checks with upstream successful, substrate exit 1 before dispatch, `jvmForwarded=0`, and the diagnostic `unsupported contract marker 'unsupported_dependency_semantics'`. Kernel admission now recognizes unsupported markers after build-plan shadow lowering prefixes task inputs as `input.<name>`. This is intentionally fail-closed evidence only; regex/configuration/attribute content filters are not claimed as native solver support. | `python3 tools/corpus_runner/run.py --manifest testing/corpus/unsupported-manifest.json --contract-only --output-dir build/corpus-regex-filter-unsupported-contract-final`; `cargo test -p gradle-substrate-daemon execution_kernel --lib`; `cargo build -p gradle-substrate-daemon --bin gradle-substrate-daemon`; `JAVA_TOOL_OPTIONS="-Dorg.gradle.rust.substrate.state.dir=$TMPDIR" python3 tools/corpus_runner/run.py --manifest <single-regex-unsupported-manifest> --gradle-command "$PWD/build/gradle-under-test/bin/gradle" --daemon-binary target/debug/gradle-substrate-daemon --runbuild-authoritative --tasks clean build --timeout 300 --output-dir build/corpus-regex-filter-unsupported-only-final5 --verbose`; `python3 -m unittest tools.corpus_runner.test_run` |
