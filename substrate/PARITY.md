...
| External OSS dogfood strict gate | Added `testing/dogfood/oss-manifest.json` with immutable refs for Spring PetClinic, Spring guide REST service, Mockito, and Okio, plus runner support for fetching git sources under `build/dogfood-oss/sources` and isolated upstream/substrate workdirs. Spring guide REST service now has a supported strict `clean jar` slice: concrete Rust RunBuild executed 5 tasks (`clean`, `compileJava`, `processResources`, `classes`, `jar`) from `build-plan-cache` with 0 JVM forwards and task/output/hash/archive-entry parity, upstream=3466ms and substrate=3833ms. Spring PetClinic remains supported for strict `clean compileJava`: 2 Rust tasks from `build-plan-cache`, 0 JVM forwards, task/output/hash/archive parity, upstream=4161ms and substrate=4337ms. The broader Spring `clean testClasses` slice is tracked as a separate fail-closed gate with a precise CycloneDX SBOM diagnostic because `:cyclonedxBom` and `:cyclonedxDirectBom` declare SBOM outputs and cannot be treated as lifecycle/no-op tasks. Mockito and Okio pass the previous coarse settings/includeBuild/buildSrc gate and fail closed at selected-task admission on grouped Kotlin/build-logic capabilities: native Kotlin compilation or a Rust-controlled Kotlin compiler worker contract, precompiled Kotlin DSL plugin generation, Gradle plugin descriptor generation, and Kotlin Gradle plugin diagnostics. Aggregate OSS dogfood evidence: 5/5 projects matched, 2/2 supported matched, 3/3 fail-closed matched, 2/2 supported zero JVM forwards, 2/5 Rust RunBuild executions, 5/5 task-graph captures, upstream task total 38, substrate task total 16, observed wall time upstream=34338ms and substrate=111348ms. | `cargo test -p gradle-substrate-daemon test_convention_jar_collects_classes_when_mappings_only_have_directories --lib`; `cargo test -p gradle-substrate-daemon task_executor::jar --lib`; `cargo test -p gradle-substrate-daemon task_graph --lib`; `cargo build -p gradle-substrate-daemon`; `python3 tools/dogfood_runner/run.py --manifest build/dogfood-oss-probe/gs-rest-jar.json --execute --gradle-command "$PWD/build/gradle-under-test/bin/gradle" --daemon-binary target/debug/gradle-substrate-daemon --output-dir build/dogfood-oss-probe/gs-rest-jar --verbose`; `python3 tools/dogfood_runner/run.py --manifest testing/dogfood/oss-manifest.json --execute --gradle-command "$PWD/build/gradle-under-test/bin/gradle" --daemon-binary target/debug/gradle-substrate-daemon --output-dir build/dogfood-oss --verbose`; artifacts: `build/dogfood-oss/dogfood-results.json`, `build/dogfood-oss/dogfood-summary.md`, `docs/rust-substrate-dogfood.md` |
