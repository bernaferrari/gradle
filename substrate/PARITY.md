...
unsupported metadata-source shapes. Repository content filters are also marked
unsupported when Gradle has include/exclude content specs, configuration gates,
or required attributes, because Rust does not yet model those repository-selection
rules. Custom repository metadata suppliers and component version listers are
...
| Repository content filters fail closed | Java bridge regressions prove both durable build-plan capture and session prefetch capture reject Maven repositories with Gradle content filters instead of admitting them as unfiltered repositories. Markers distinguish include/exclude content specs, included/excluded configurations, and required attributes. Java bridge dependency adapter suite passed. | `./gradlew :rust-bridge:test --tests org.gradle.internal.rustbridge.dependency.DependencyResolutionModelAdapterTest --no-daemon --console=plain` |
