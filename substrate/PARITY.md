...
| Classifier artifact corpus | 1/1 matched, 12/12 task parity, no-fallback, output/hash/archive parity, declared graph includes `org.apache.commons:commons-lang3:3.14.0:sources@jar`, observed wall time upstream=6955ms and substrate=3620ms | `python3 tools/corpus_runner/run.py --project "$PWD/testing/corpus/dependency-classifier-artifact-kotlin-dsl" --gradle-command "$PWD/build/gradle-under-test/bin/gradle" --daemon-binary target/debug/gradle-substrate-daemon --runbuild-authoritative --dependency-graph-parity --tasks clean build --timeout 300 --output-dir build/corpus-dependency-classifier-artifact-6 --verbose` |
