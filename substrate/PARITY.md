...
- The checked-in offline corpus covers Java library, Java application,
  ...
  resolved-file report, and an OSS-style Java library slice with sources
  JAR/Javadoc/report outputs.
...
- `testing/corpus/unsupported-manifest.json` tracks work that must remain
  ...
  covers artifact transforms and composite substitution. Exact method-level Test
  ...
  fixture, and exact lenient artifact-view resolved-file report fixture are now
  ...
  actions, arbitrary custom task actions, and artifact-view selection semantics
  beyond the exact lenient report fixture still belong in unsupported gates.
...
| Unsupported artifact transforms | Unsupported corpus now includes `artifact-transform-unsupported-kotlin-dsl`, a checked-in local Maven fixture that registers a `TransformAction` through `dependencies.registerTransform(...)`, resolves the transformed runtime classpath from a custom task, and writes deterministic output under `build/artifact-transform`. Contract scanning reports `artifact-transform`; bridge task contracts mark artifact-transform registration as unsupported dependency semantics with `artifact-transform:build-script`; and the authoritative expected-fail-closed run passed with upstream successful, substrate exit 1 before dispatch, `fail_closed_message=true`, no noop fallback, and zero JVM task forwards. This is intentionally fail-closed evidence only; artifact transform attributes, parameters, cache keys, and transform execution are not claimed as native solver support. | `./gradlew -p testing/corpus/artifact-transform-unsupported-kotlin-dsl clean build --no-daemon --console=plain`; `python3 tools/corpus_runner/run.py --manifest testing/corpus/unsupported-manifest.json --contract-only --output-dir build/corpus-artifact-transform-unsupported-contract-final`; `python3 -m unittest tools.corpus_runner.test_run`; `./gradlew :rust-bridge:compileJava --no-daemon --console=plain`; `cargo test -p gradle-substrate-daemon execution_kernel --lib`; `./gradlew :distributions-full:install -Pgradle_installPath=$PWD/build/gradle-under-test -Dorg.gradle.unsafe.isolated-projects=false -Dorg.gradle.configuration-cache=false --no-daemon --console=plain`; `JAVA_TOOL_OPTIONS="-Dorg.gradle.rust.substrate.state.dir=$PWD/build/artifact-transform-fail-closed-state-3" python3 tools/corpus_runner/run.py --manifest build/artifact-transform-single-manifest.json --gradle-command "$PWD/build/gradle-under-test/bin/gradle" --daemon-binary target/debug/gradle-substrate-daemon --runbuild-authoritative --tasks clean build --timeout 300 --output-dir build/corpus-artifact-transform-unsupported-only-manifest --verbose`; focused `:rust-bridge:test` is currently blocked before test execution by an unrelated `:rust-bridge:testCompileProtoPath` dependency-version conflict between JUnit/Spock/Objenesis constraints. |
...
| Corpus runner tests | 43 passed | `python3 -m unittest tools.corpus_runner.test_run` |
