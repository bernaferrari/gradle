...
- The checked-in offline corpus covers Java library, Java application,
  ...
  custom static `writeText` report task, and an OSS-style Java library slice
  with sources JAR/Javadoc/report outputs.
...
- `testing/corpus/unsupported-manifest.json` tracks work that must remain
  ...
  covers artifact views, artifact transforms, and composite substitution. Exact
  method-level Test
  ...
  resolved-file report fixture, and exact static custom `writeText` report task
  fixture are now represented natively;
  ...
  actions, and arbitrary custom task actions still belong in unsupported gates.
...
| Unsupported artifact views | Unsupported corpus now includes `artifact-view-unsupported-kotlin-dsl`, a checked-in local Maven fixture that resolves `runtimeClasspath.incoming.artifactView { ... }` from a custom task and writes deterministic output. Contract scanning reports `artifact-view`; bridge task contracts mark artifact view usage as unsupported dependency semantics; and the authoritative expected-fail-closed run passed with upstream successful, substrate exit 1 before dispatch, `fail_closed_message=true`, `artifact-view` in the diagnostic, and `jvmForwarded=0`. This is intentionally fail-closed evidence only; artifact views/transforms are not claimed as native solver support until the Rust graph builder models their selection semantics explicitly. | `./gradlew -p testing/corpus/artifact-view-unsupported-kotlin-dsl clean build --no-daemon --console=plain`; `python3 tools/corpus_runner/run.py --manifest testing/corpus/unsupported-manifest.json --contract-only --output-dir build/corpus-artifact-view-unsupported-contract`; `python3 -m unittest tools.corpus_runner.test_run`; `./gradlew :rust-bridge:compileJava --no-daemon --console=plain`; `cargo test -p gradle-substrate-daemon execution_kernel --lib`; `./gradlew :distributions-full:install -Pgradle_installPath=$PWD/build/gradle-under-test -Dorg.gradle.unsafe.isolated-projects=false -Dorg.gradle.configuration-cache=false --no-daemon --console=plain`; `JAVA_TOOL_OPTIONS="-Dorg.gradle.rust.substrate.state.dir=$TMPDIR" python3 tools/corpus_runner/run.py --manifest <single-artifact-view-unsupported-manifest> --gradle-command "$PWD/build/gradle-under-test/bin/gradle" --daemon-binary target/debug/gradle-substrate-daemon --runbuild-authoritative --tasks clean build --timeout 300 --output-dir build/corpus-artifact-view-unsupported-only --verbose` |
...
| Corpus runner tests | 41 passed | `python3 -m unittest tools.corpus_runner.test_run` |
