...
| External OSS dogfood strict gate | Added `testing/dogfood/oss-manifest.json` with immutable refs for Spring PetClinic, Mockito, and Okio, plus runner support for fetching git sources under `build/dogfood-oss/sources` and isolated upstream/substrate workdirs. The runner now requires concrete `Rust executed N` RunBuild evidence before counting a supported entry. Current OSS dogfood execution is intentionally strict fail-closed: 3/3 projects reject before Rust execution with `composite-substitution:settings`, 3/3 task-graph captures, 0/3 Rust RunBuild executions, upstream task total 31, substrate task total 9, observed wall time upstream=24439ms and substrate=18441ms. Earlier native-ready OSS smoke runs delegated before concrete Rust execution and are no longer counted as support evidence. | `python3 tools/dogfood_runner/run.py --manifest testing/dogfood/oss-manifest.json --list`; `python3 tools/dogfood_runner/run.py --manifest testing/dogfood/oss-manifest.json --fetch-only --output-dir build/dogfood-oss`; `python3 tools/dogfood_runner/run.py --manifest testing/dogfood/oss-manifest.json --execute --gradle-command "$PWD/build/gradle-under-test/bin/gradle" --daemon-binary target/debug/gradle-substrate-daemon --output-dir build/dogfood-oss --verbose`; artifacts: `build/dogfood-oss/dogfood-results.json`, `build/dogfood-oss/dogfood-summary.md`, `docs/rust-substrate-dogfood.md`; follow-up: `gradle-fork-1up` |
