...
`fail_on_conflict` direct conflicts now have checked-in gates; broader conflict
...
| Conflict fail-closed strategy | Service-level regression proves `resolution_strategy.strategy=fail_on_conflict` returns `success=false` with a precise conflicting coordinate/version diagnostic instead of logging and silently selecting the highest version. Dependency resolution suite: 127 passed, 0 failed. Full daemon library suite: 1628 passed, 0 failed, 3 ignored. | `cargo test -p gradle-substrate-daemon fail_on_conflict --lib -- --nocapture`; `cargo test -p gradle-substrate-daemon dependency_resolution --lib`; `cargo test -p gradle-substrate-daemon --lib` |
