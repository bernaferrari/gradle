...
| External OSS dogfood smoke | Added `testing/dogfood/oss-manifest.json` with immutable refs for Spring PetClinic, Mockito, and Okio, plus runner support for fetching git sources under `build/dogfood-oss/sources`. Current OSS dogfood execution passed 3/3 in `native-ready-default` mode with zero JVM forwards: Spring PetClinic runs `clean classes` with task/output parity and a documented generated-SBOM hash allowlist; Mockito and Okio run selected `help` smokes. Evidence: 3/3 Rust RunBuild markers, 3/3 task-graph captures, upstream task total 28, substrate task total 28, observed wall time upstream=21331ms and substrate=29769ms. This is selected-task admission/parity evidence for recognizable OSS builds, not full build parity for those projects. | `python3 tools/dogfood_runner/run.py --manifest testing/dogfood/oss-manifest.json --list`; `python3 tools/dogfood_runner/run.py --manifest testing/dogfood/oss-manifest.json --fetch-only --output-dir build/dogfood-oss`; `python3 tools/dogfood_runner/run.py --manifest testing/dogfood/oss-manifest.json --execute --gradle-command "$PWD/build/gradle-under-test/bin/gradle" --daemon-binary target/debug/gradle-substrate-daemon --output-dir build/dogfood-oss --verbose`; artifacts: `build/dogfood-oss/dogfood-results.json`, `build/dogfood-oss/dogfood-summary.md`, `docs/rust-substrate-dogfood.md` |
