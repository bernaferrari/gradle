...
| External OSS dogfood strict gate | Added `testing/dogfood/oss-manifest.json` with immutable refs for Spring PetClinic, Spring guide REST service, Mockito, and Okio, plus runner support for fetching git sources under `build/dogfood-oss/sources` and isolated upstream/substrate workdirs. Spring guide REST service now has a supported strict `clean classes` slice: concrete Rust RunBuild executed 4 tasks (`clean`, `compileJava`, `processResources`, `classes`) from `build-plan-cache` with 0 JVM forwards and task/output/hash/archive parity, upstream=4029ms and substrate=3803ms. Spring PetClinic remains supported for strict `clean compileJava`: 2 Rust tasks from `build-plan-cache`, 0 JVM forwards, task/output/hash/archive parity, upstream=4281ms and substrate=4412ms. The broader Spring `clean testClasses` slice is tracked as a separate fail-closed gate with a precise CycloneDX SBOM diagnostic because `:cyclonedxBom` and `:cyclonedxDirectBom` declare SBOM outputs and cannot be treated as lifecycle/no-op tasks. Mockito and Okio pass the previous coarse settings/includeBuild/buildSrc gate and fail closed at selected-task admission on grouped Kotlin/build-logic capabilities: native Kotlin compilation or a Rust-controlled Kotlin compiler worker contract, precompiled Kotlin DSL plugin generation, Gradle plugin descriptor generation, and Kotlin Gradle plugin diagnostics. Aggregate OSS dogfood evidence: 5/5 projects matched, 2/2 supported matched, 3/3 fail-closed matched, 2/2 supported zero JVM forwards, 2/5 Rust RunBuild executions, 5/5 task-graph captures, upstream task total 37, substrate task total 15, observed wall time upstream=34098ms and substrate=109853ms. | `cargo test -p gradle-substrate-daemon rejects_kotlin_build_logic_tasks_with_grouped_capability_diagnostics --lib`; `cargo test -p gradle-substrate-daemon execution_kernel --lib`; `cargo test -p gradle-substrate-daemon task_graph --lib`; `./gradlew :rust-bridge:compileJava :rust-bridge:compileTestJava -x :rust-bridge:extractIncludeTestProto -x :rust-bridge:generateTestProto --no-daemon --console=plain`; `cargo build -p gradle-substrate-daemon`; `./gradlew :distributions-full:install -Pgradle_installPath=$PWD/build/gradle-under-test -Dorg.gradle.unsafe.isolated-projects=false -Dorg.gradle.configuration-cache=false --no-daemon --console=plain`; `python3 tools/dogfood_runner/run.py --manifest testing/dogfood/oss-manifest.json --execute --gradle-command "$PWD/build/gradle-under-test/bin/gradle" --daemon-binary target/debug/gradle-substrate-daemon --output-dir build/dogfood-oss --verbose`; artifacts: `build/dogfood-oss/dogfood-results.json`, `build/dogfood-oss/dogfood-summary.md`, `docs/rust-substrate-dogfood.md` |
