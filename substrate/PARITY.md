...
| Dependency descriptor unsupported feature gates | Graph-builder regression proves Rust rejects changing modules, direct optional dependency flags, and Ivy conf mappings before repository traversal, because those request-level semantics are not represented in the current native solver contract. Dependency resolution suite: 128 passed, 0 failed. Full daemon library suite: 1654 passed, 0 failed, 3 ignored. | `cargo test -p gradle-substrate-daemon dependency_solver::graph_builder --lib`; `cargo test -p gradle-substrate-daemon dependency_resolution::tests --lib`; `cargo test -p gradle-substrate-daemon --lib` |
