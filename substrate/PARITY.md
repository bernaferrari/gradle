...
| Gradle Module Metadata rich static versions | Parser/service regressions prove Rust supports static selected-variant `strictly`, `prefers`, and `rejects` in GMM dependency and dependency-constraint versions, upgrades a selected dependency through a static strict constraint, rejects selected rejected versions, and fails closed on dynamic rejected selectors. This is GMM-only; direct Gradle rich-version bridge capture is still not claimed. Focused Gradle metadata tests: 19 passed. Dependency resolution suite: 130 passed, 0 failed. Full daemon library suite: 1665 passed, 0 failed, 3 ignored. | `cargo test -p gradle-substrate-daemon gradle_module_metadata --lib`; `cargo test -p gradle-substrate-daemon dependency_resolution::tests --lib`; `cargo test -p gradle-substrate-daemon --lib` |
