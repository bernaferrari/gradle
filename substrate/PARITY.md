...
| CycloneDX SBOM IR foundation | Rust now has an explicit `gradle-substrate.cyclonedx-sbom.v1` contract type plus deterministic JSON and XML renderers. Kernel admission rejects any attempted native `CyclonedxDirectTask` or `CyclonedxAggregateTask` unless the execution context carries a base64-encoded schema-backed SBOM contract in `input_value.sbom_contract_json_b64` or `input.sbom_contract_json_b64`. This is a safety foundation only: the JVM bridge does not yet capture this complete IR from real Gradle/CycloneDX tasks, so Spring PetClinic `testClasses` remains fail-closed until bridge capture and output parity exist. | `cargo test -p gradle-substrate-daemon cyclonedx --lib` passed 6/6; `cargo test -p gradle-substrate-daemon execution_kernel --lib` passed 16/16. |
