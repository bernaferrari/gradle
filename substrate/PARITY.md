...
back to Maven POM resolution. Custom capabilities, ambiguous JVM variants, and
rich variant dependency versions fail closed instead of being approximated.
...
| Gradle Module Metadata JVM variant slice | Parser/service tests passed; Rust selects the `runtimeElements` dependency list for an explicitly enabled Gradle metadata repository, does not fetch the root POM, and fails closed on custom capabilities | `cargo test -p gradle-substrate-daemon gradle_module_metadata --lib -- --nocapture`; `cargo test -p gradle-substrate-daemon dependency_resolution --lib`; `cargo test -p gradle-substrate-daemon --lib` |
| Gradle Module Metadata public corpus | 1/1 matched, 12/12 task parity, no-fallback, declared and resolved graph parity enabled, output/hash/archive parity passed for `com.squareup.okhttp3:okhttp:4.12.0` with `metadataSources { gradleMetadata(); mavenPom() }`, observed wall time upstream=6034ms and substrate=3448ms | `python3 tools/corpus_runner/run.py --project "$PWD/testing/corpus/dependency-gradle-metadata-kotlin-dsl" --gradle-command "$PWD/build/gradle-under-test/bin/gradle" --daemon-binary target/debug/gradle-substrate-daemon --runbuild-authoritative --dependency-graph-parity --resolved-dependency-graph-parity --tasks clean build --timeout 300 --output-dir build/corpus-gradle-metadata-parity --verbose` |
