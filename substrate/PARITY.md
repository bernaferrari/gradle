...
unsupported metadata-source shapes. Static exact Maven repository
`includeGroup`/`excludeGroup` content filters are captured as native repository
group filters and Rust skips repositories that do not admit a dependency group.
Other content filters are still marked unsupported, including module/version,
regex/subgroup, configuration, and required-attribute gates. Custom repository
...
| Static repository group content filters | Proto/bridge regressions prove exact Gradle Maven `includeGroup`/`excludeGroup` content specs are captured as repository `include_groups`/`exclude_groups`, while regex, subgroup, module/version, configuration, and attribute filters remain fail-closed. Rust resolver regression proves a repository whose include groups do not admit the dependency group is skipped before POM fetch and artifact URL selection uses the admitted repository. Dependency resolution suite: 134 passed, 0 failed. Full daemon library suite: 1673 passed, 0 failed, 3 ignored. Java bridge dependency adapter suite passed. | `./gradlew :rust-bridge:test --tests org.gradle.internal.rustbridge.dependency.DependencyResolutionModelAdapterTest --no-daemon --console=plain`; `cargo test -p gradle-substrate-daemon test_repository_group_content_filter_skips_non_matching_repo --lib`; `cargo test -p gradle-substrate-daemon dependency_resolution::tests --lib`; `cargo test -p gradle-substrate-daemon --lib` |
