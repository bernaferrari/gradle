...
semantics, dependency constraints, BOM/platform semantics, rich versions, SNAPSHOTs,
...
| Gradle Module Metadata unsupported fields | Parser regressions prove Rust fails closed when a selected JVM variant uses `available-at` redirects, variant dependency constraints, or dependency exclusions, rather than silently discarding those graph-affecting fields during serde parsing. Dependency resolution suite: 128 passed, 0 failed. Full daemon library suite: 1653 passed, 0 failed, 3 ignored. | `cargo test -p gradle-substrate-daemon gradle_module_metadata --lib`; `cargo test -p gradle-substrate-daemon dependency_resolution::tests --lib`; `cargo test -p gradle-substrate-daemon --lib` |
